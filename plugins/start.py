    ...
    if len(text) > 7:
        ...
        if len(argument) == 3:
            ...
            try:
                ...
                return
        temp_msg = await message.reply("<code>Tunggu Bentar ya...</code>")
        try:
...
async def get_users(client: Bot, message: Message):
    msg = await client.send_message(
        chat_id=message.chat.id, text="<code>Masih proses ...</code>"
    )
    users = await full_userbase()
    await msg.edit(f"{len(users)} <b>Pengguna sudah menggunakan bot ini</b>")
...
async def send_text(client: Bot, message: Message):
    if message.reply_to_message:
        ...
        pls_wait = await message.reply(
            "<code>Broadcasting Message Tunggu Bentar...</code>"
        )
        for row in query:
            chat_id = int(row[0])
            try:
